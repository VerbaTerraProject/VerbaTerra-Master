...
- **Hierarchy term restored:** The original implementation split `_safe_norm(df["hierarchy"])` across lines, which evaluated `_safe_` as a missing name. The refactor reunites the call so hierarchy properly contributes to NLIS.
- **Schema-backed validation:** Inputs now route through `verbaterra.dataio.validate_frame`, guaranteeing the expected columns and numeric dtypes before any score is produced.
